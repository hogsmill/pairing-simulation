<template>
  <div class="card bg-light mb-3 col-md-3 no-padding-r-l run-type" v-if="!(gameState.running && gameState.runType == 'Full Run')">
    <div class="card-body">
      ...
      <div class="radio">
        <input type="radio" id="fullRun" name="runType" v-model="gameState.runType" :value="'Full Run'">
        <label for="fullRun">Full Run</label>
      </div>
      <div class="radio">
        <input type="radio" id="stepThrough" name="runType" v-model="gameState.runType" :value="'Step Through'">
        <label for="stepThrough">Step-Through</label>
      </div>
      <button @click="run()" class="btn btn-site-primary start" :disabled="gameState.running || ! gameState.backlogCreated">
        Go
      </button>
    </div>
  </div>
  ...
  methods: {
    ...
    boardsComplete() {
      ...
      for (const strategy in this.gameState.strategies) {
        const board = this.gameState.strategies[strategy]
        if (board.run) {
    ...
    run() {
      ...
      if (! this.boardsComplete()) {
        ...
        for (const strategy in state.strategies) {
